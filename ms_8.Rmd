...
# compare % of people that're obese to % of people that're lia and la
...
food_sum <- food_access %>% 
    group_by(state, urban, lia_pop, la_pop, region) %>% 
    summarise(total_pop = sum(popul),
           total_lia_pop = sum(lia_pop),
           total_la_pop = sum(la_pop),
           perc_lia = round((total_lia_pop/total_pop)*100, digits = 0),
           perc_la = round((total_la_pop/total_pop)*100, digits = 0),
           mean_poverty = round(mean(poverty), digits = 0)) %>% 
    select(-total_pop, -total_lia_pop, -total_la_pop, -la_pop, -lia_pop)
