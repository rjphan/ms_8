...
food_poverty_mean_plot <- plot_usmap(data = food_poverty_mean, values = "mean_poverty", 
                                     color = "green", labels = FALSE) +
    theme(legend.position = "right",
          plot.title = element_text(hjust = 0.5)) + 
    scale_fill_continuous(low = "white", high = "green", name = "Mean Poverty Rate") +
...
# food_lia$perc =  paste(round(100*food_lia$perc, digits = 0), "%", sep="") 

food_lia_plot <- food_lia %>% 
    group_by(region) %>% 
    ggplot(aes(x = reorder(state, perc), y = perc, fill = region)) +
    geom_col() +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 65, hjust = 0.3, size = 7),
          panel.grid.major.x = element_line(color = "white"),
          plot.subtitle = element_text(size = 8)) +
    labs(title = "Percent of Counties with a Lack of Food Access",
         subtitle = "Lack of Food Access Defined by 33% of the Population Living More Than \n 1 Mile (Urban) Or 10 Miles (Rural) Away from Supermarket, Supercenter, or Grocery Store",
         caption = "Data from US Department of Agriculture and 2010 Census Bureau",
         x = "State",
